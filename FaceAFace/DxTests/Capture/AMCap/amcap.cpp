...
void MakeMenuOptions()
{
    ...
    if(gcap.pDlg && SUCCEEDED(gcap.pDlg->HasDialog(VfwCaptureDialog_Format)))
    {
        AppendMenu(hMenuSub, MF_STRING, MENU_DIALOG0 + zz, TEXT("Video Format..."));
        gcap.iFormatDialogPos = zz++;
    }
    if(gcap.pDlg && SUCCEEDED(gcap.pDlg->HasDialog(VfwCaptureDialog_Source)))
    {
        AppendMenu(hMenuSub, MF_STRING, MENU_DIALOG0 + zz, TEXT("Video Source..."));
        gcap.iSourceDialogPos = zz++;
    }
    if(gcap.pDlg && SUCCEEDED(gcap.pDlg->HasDialog(VfwCaptureDialog_Display)))
    {
